...
- [x] **Decentralized Data Architectures**: a direction rather than a single feature, composing several primitives — Merkle commitments for trust-minimised sync, three-way merge for federated schema governance, the `SagoService` gRPC interface (including Merkle sync RPCs), and per-target/per-domain `domain`/`owner`/RBAC/`endpoint` metadata in config. All four consumer-facing follow-ups below are now shipped; see [DECENTRALIZED.md](./DECENTRALIZED.md) for the full design note.
  ...
  - [x] Domain discovery (config-declared registry: `[domains.<name>].endpoint` in `Sago.toml`, resolved via `sago-core::registry` and listed with `sago domains`).
